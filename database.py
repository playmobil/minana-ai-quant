...
class Database:
    ...
    def init_db(self):
        ...
        if 'provider_type' not in columns:
            print("[MIGRATION] Adding provider_type column to providers table")
            cursor.execute('''
                ALTER TABLE providers ADD COLUMN provider_type TEXT DEFAULT 'openai'
            ''')
            # Update existing providers based on their API URL
            cursor.execute('''
                UPDATE providers
                SET provider_type = CASE
                    WHEN api_url LIKE '%deepseek%' THEN 'deepseek'
                    WHEN api_url LIKE '%anthropic%' OR api_url LIKE '%claude%' THEN 'anthropic'
                    WHEN api_url LIKE '%gemini%' OR api_url LIKE '%google%' THEN 'gemini'
                    ELSE 'openai'
                END
            ''')
            print("[MIGRATION] provider_type column added and populated")
    ...
    def add_provider(self, name: str, api_url: str, api_key: str, models: str = '', provider_type: str = 'openai') -> int:
        ...
        cursor = conn.cursor()
        cursor.execute('''
            INSERT INTO providers (name, provider_type, api_url, api_key, models)
            VALUES (?, ?, ?, ?, ?)
        ''', (name, provider_type, api_url, api_key, models))
    ...
    def update_provider(self, provider_id: int, name: str, api_url: str, api_key: str, models: str, provider_type: str = 'openai'):
        ...
        cursor.execute('''
            UPDATE providers
            SET name = ?, provider_type = ?, api_url = ?, api_key = ?, models = ?
            WHERE id = ?
        ''', (name, provider_type, api_url, api_key, models, provider_id))
        conn.commit()
        conn.close()
